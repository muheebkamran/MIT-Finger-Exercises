# for running unit tests on 6.100A/L student code[cite: 2]
...
import re
import random
# pulled from http://stackoverflow.com/questions/20567497/overwrite-built-in-function[cite: 2]
outputstr=""
...
# documentation: https://docs.python.org/3/library/unittest.html#unittest.TestCase[cite: 2]
class TestPS2(unittest.TestCase):

    # TODO Add HangmanOracle tests[cite: 2]
        ...
        with unittest.mock.patch('builtins.input',  make_input):
            ...
            try:
                ...
                if len(lines) > 9:
                    ...
                    self.assertTrue("revealed" in lines[7]) # lost 2 guesses[cite: 2]
                    self.assertTrue("6 guesses" in lines[8])
                    self.assertTrue("revealed" in lines[8]) # lost 2 guesses[cite: 2]
...
# messages you'd like the student to see if the test fails.[cite: 2]
failure_messages = {
...
# Dictionary mapping function names from the above TestCase class to
# messages you'd like the student to see if their code throws an error.[cite: 2]
error_messages = {
...
# Dictionary mapping function names from the above TestCase class to
# the point value each test is worth. Make sure these add up to 5![cite: 2]
# TODO update values depending on the number of Hangman test cases we use[cite: 2]
point_values = {
...
# grade comment for a suit of unit tests[cite: 2]
class Results_600(unittest.TextTestResult):

    # We override the init method so that the Result object[cite: 2]
    # can store the score and appropriate test output.[cite: 2]
...
if __name__ == '__main__':
    ...
    # os.remove("run_game_test_results.txt")[cite: 2]
